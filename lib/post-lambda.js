...
var jsonpathUtil = require('./jsonpath-util.js');
...
exports.intercept = function (service, target, event, callback) {
  if (target.jsonpath) {
    var errors = [];
    event.Records = event.Records.filter(function (record) {
      return jsonpathUtil.jsonPathTest(record, target, errors, record.kinesis.data);
    });
  }
