 ...
 *  - jsonpath    (String)  [optional]: JsonPath filter for the fan-out target
 */
...
function readTarget(record, serviceDefinitions) {
  ...
  if((target.jsonpath !== null) && ((typeof target.jsonpath) != "string") && jp.parse(target.jsonpath)) {
    console.error("Invalid configuration for target '" + target.id + "' of type '" + target.type + "', 'jsonpath' property must be a valid JsonPath String. Ignoring target");
    return null;
  }
  return target;
}
